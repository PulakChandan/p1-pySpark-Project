...
class LinkedList:
    ...
    def findMax(self):
        temp=self.__head
        maxNode=Node(temp.get_data())
        print(type(maxNode.get_data()),type(temp.get_data()))
        while temp is not None:
            if maxNode.get_data()<temp.get_data():
                maxNode.set_data(temp.get_data)
            temp=temp.get_next()
        return maxNode.get_data()
...
position=list1.find_pos("Five Star")
if(position==-1):
    print("Node not found!!")
else:
    print("Node found at position:",position) 
node=list1.find_node("Snickers")
if node is None:
    print("Node not found!!")
else:
    print("Node found!")
